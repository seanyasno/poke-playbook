...
import { mockTeam, createMockTeam } from '../../../../../../test/mocks/data/teams.mock';
...
vi.mock('../../../../../../features', async (importOriginal) => {
  ...
  return {
    ...actual,
    TeamCardMenu: ({ teamId, teamName }: { teamId: string; teamName: string }) => (
      <div data-testid="team-card-menu" data-team-id={teamId} data-team-name={teamName}>
        Menu
      </div>
    ),
    PokemonSprites: ({ pokemon }: { pokemon: Array<{ pokemon_id: number; pokemon_name: string }> }) => (
...
describe('TeamCard', () => {
  ...
  describe('Layout and styling', () => {
    it('should have correct layout structure', () => {
      ...
      expect(header).toHaveClass('flex', 'justify-between', 'items-start', 'mb-3');
  ...
  describe('Edge cases', () => {
    ...
    it('should handle very long descriptions gracefully', () => {
      const teamWithLongDescription = createMockTeam({
        description: 'This is a very long description that should be truncated with ellipsis when it exceeds two lines in the container. It should handle multiple lines of text gracefully and maintain proper spacing.',
      });
      
      renderTeamCard(teamWithLongDescription);
      
      const description = screen.getByText(teamWithLongDescription.description!);
